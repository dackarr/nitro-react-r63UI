...
import { TABS, TAB_ACHIEVEMENTS, TAB_BADGES, TAB_BOTS, TAB_FURNITURE, TAB_PETS, UNSEEN_CATEGORIES, } from './constants';
import { InventoryAchievementView } from "./views/achievements/InventoryAchievementView";
...
export const InventoryView: FC<{}> = props =>
{
    ...
    return (
        <NitroCardView uniqueKey={ 'inventory' } className={ `${ isTrading ? 'nitro-inventory trading  no-resize' : 'nitro-inventory' }` } theme={ isTrading ? 'messenger' : 'messenger' } >
            <NitroCardHeaderView headerText={ LocalizeText('inventory.title') } hideButtonClose={ true } onCloseClick={ onClose } />
            <>
                <NitroCardTabsView>
                    { TABS.map((name, index) =>
                    {
