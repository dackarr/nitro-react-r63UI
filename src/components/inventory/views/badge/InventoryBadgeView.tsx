        ...
        <div className="badge-wrapper">
            <Grid gap={ 2 } className="badges-list">
                <Column size={ 8 } overflow="hidden">
                    ...
                    <AutoGrid className="badges-overflow-x" gap={ 1 } columnCount={ 5 }>
            ...
            { !!selectedBadgeCode &&
                <Flex className="bg-white pb-1 px-2 rounded mt-2" style={ { height: '50px' } } justifyContent={ 'between' } alignItems={ 'end' } gap={ 2 }>
                    ...
                    <Button variant="illumina" className="mb-2" style={ { fontSize: '12px' } } disabled={ !isWearingBadge(selectedBadgeCode) && !canWearBadges() } onClick={ event => toggleBadge(selectedBadgeCode) }>{ LocalizeText(isWearingBadge(selectedBadgeCode) ? 'inventory.badges.clearbadge' : 'inventory.badges.wearbadge') }</Button>
                </Flex> }
            <div className="nitro-progress-bar border-0 text-white mt-1">
