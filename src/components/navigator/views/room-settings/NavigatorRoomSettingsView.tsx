...
export const NavigatorRoomSettingsView: FC<{}> = props =>
{
    ...
    return (
        <NitroCardView uniqueKey="nitro-room-settings" className="nitro-room-settings">
            <NitroCardHeaderView headerText={ LocalizeText('navigator.roomsettings') } isInfoToHabboPages={ (currentTab === TABS[3]) ? true : false } onClickInfoHabboPages={ () => (currentTab === TABS[3]) ? CreateLinkEvent('habbopages/chat/options.html') : null }onCloseClick={ onClose } />
            <NitroCardTabsView>
                { TABS.map(tab =>
                {
