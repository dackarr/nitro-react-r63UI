...
export const FriendBarItemView: FC<{ friend: MessengerFriend }> = props =>
{
    ...
    {
        return (
            <div ref={ elementRef } className={ 'btn find-friends friend-bar-item friend-bar-search ' + (isVisible ? 'bottom-5 notfriend-bar-item-active' : '') } onClick={ event => setVisible(prevValue => !prevValue) }>
                <div className="friend-bar-item-head position-absolute"/>
                <div className="friend-bar-text mt-1">{ LocalizeText('friend.bar.find.title') }</div>
                { isVisible &&
    ...
    return (
        <div ref={ elementRef } className={ 'btn find-friends-active friend-bar-item ' + (isVisible ? 'friend-bar-item-active' : '') } onClick={ event => setVisible(prevValue => !prevValue) }>
            <div className={ `friend-bar-item-head z-3 position-relative position-absolute ${ friend.id > 0 ? 'avatar': 'group' }` }>
