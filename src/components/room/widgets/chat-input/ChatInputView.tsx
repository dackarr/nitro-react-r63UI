...
import { Base, Button, Text } from '../../../../common';
        ...
        createPortal(
            <>
                <div className="nitro-chat-input-container" onMouseEnter={ () => setShowInfohabboPages(true) } onMouseLeave={ () => setTimeout(() => setShowInfohabboPages(false), 100) }>
                    ...
                    { (showInfoHabboPages) &&
                        <Base className="info-habbopages" onClick={ () => CreateLinkEvent('habbopages/chat/chatting.html') }></Base>
                    }
                    <Button className="font-bold say-btn" onClick={ () => sendChatValue(chatValue) }>{ LocalizeText('widgets.chatinput.say') }</Button>
                </div>
            </>,
