...
export const ChatInputStyleSelectorView: FC<ChatInputStyleSelectorViewProps> = props =>
{
    ...
    return (
        <>
            ...
            <Flex pointer className="chatstyles-selector cursor-pointer" onClick={ toggleSelector }>
                <Base className="bubble-container">
                    <Base className={ `chat-bubble selector bubble-${ chatStyleId }` }>&nbsp;</Base>
                </Base>
                <Base className={ selectorVisible ? 'arrow-selector displayed' : 'arrow-selector' }>&nbsp;</Base>
            </Flex>
            <Overlay show={ selectorVisible } target={ target } placement="top">
                <Popover id="arrow-none" className="nitro-chat-style-selector-container">
